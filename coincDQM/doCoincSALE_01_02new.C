...
// (4)
// telescope settings
Float_t angle = -157;//deg
Float_t distance=853;
...
Float_t phi1Corr = 105;// in degrees (the one stored in the header + refinements)
Float_t phi2Corr = 17;// in degrees

Float_t doCoinc(const char *fileIn="coincSALE_0102n.root",TCanvas *cout=NULL,Float_t &rate,Float_t &rateErr){
  ...
  TFile *fo = new TFile("outputSALE-01-02.root","RECREATE");
  h->Write();
  hDeltaTheta->Write();
  hDeltaPhi->Write();
  hThetaRel->Write();
